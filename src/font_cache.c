...
void drop_font_cache(FontCache *cache) {
    ...
    if (cache->bitmap_cache) {
        drop_hashmap(cache->bitmap_cache);
    }
...
bool make_bitmap(FontCache *font_cache, wchar_t c) {
    ...
    *key = c;
    ...
    glyph_map->bitmap = bitmap;

    glyph_map->bitmap_left = font_cache->face->glyph->bitmap_left;
    glyph_map->bitmap_top = font_cache->face->glyph->bitmap_top;
    ...
    if (bitmap->width > font_cache->max_width) {
        font_cache->max_width = bitmap->width;
    }

    if (bitmap->rows > font_cache->max_height) {
        font_cache->max_height = bitmap->rows;
    }
...
bool init_face_cache(FontCache *cache, char face_path[], int point_size,
    ...
    return make_bitmaps(cache, point_size, characters);
}
